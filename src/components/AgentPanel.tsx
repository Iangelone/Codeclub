...
import { Brain, Check, Download, Search, Trash2 } from 'lucide-react';
...
export default function AgentPanel({ projectPath, creature }: { projectPath?: string; creature?: React.ReactNode }) {
  ...
  const load = async () => {
    if (!projectPath) { setMemories([]); return; }
    setMemories(query ? await searchMemory(projectPath, query) : await listMemories(projectPath));
  };
  ...
  const confirmMemory = async (memory: MemoryEntry) => { if (!projectPath) return; await saveMemory(projectPath, memory.key, memory.content, memory.tags, { status: 'confirmed', confidence: Math.max(memory.confidence || 0.5, 0.8) }); await load(); };
  const forgetMemory = async (memory: MemoryEntry) => { if (!projectPath) return; await deleteMemory(projectPath, memory.key); await load(); };
  ...
  return <main className="mt-10 w-full max-w-[1040px] px-6 pb-8 lg:px-8">
    ...
    <div className="mt-8 flex flex-wrap items-center justify-between gap-3">
      <div className="flex min-w-0 items-center gap-3">
        ...
        <nav className="flex items-center gap-0.5 text-[13px] text-[#777777]" aria-label={text.filters}>
          {filters.map(([id, label]) => <button key={id} type="button" onClick={() => setFilter(id)} className={`rounded-[8px] border-0 px-3 py-1.5 ${filter === id ? 'bg-[#2b2b2b] text-[#eeeeee]' : 'bg-transparent text-[#777777] hover:bg-[#202020]'}`}>{label} {filterCount(id) > 0 && <span className="text-[#999999]">{filterCount(id)}</span>}</button>)}
        </nav>
      </div>
    ...
    {visibleMemories.length > 0 ? <section className="mt-9 grid min-w-0 gap-1.5">{visibleMemories.map((memory) => <article key={memory.key} className="flex min-h-[60px] min-w-0 items-center gap-3 overflow-hidden rounded-lg px-3 transition-colors hover:bg-[#202020]"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-[10px] border border-[#2d2d2d] bg-[#151515]"><Brain size={19} strokeWidth={1.7} className="text-[#8bc7ff]" /></div><div className="min-w-0 flex-1"><h3 className="m-0 truncate text-[14px] font-semibold text-[#eeeeee]">{memory.key}</h3><p className="mt-0.5 truncate text-[13px] text-[#888888]">{memory.content}</p></div><span className="shrink-0 text-[11px] text-[#777777]">{Math.round((memory.confidence || 0) * 100)}%</span><button type="button" onClick={() => void confirmMemory(memory)} aria-label={text.confirm} className="grid h-7 w-7 shrink-0 place-items-center rounded-md border-0 bg-transparent text-[#777777] hover:bg-[#2b2b2b] hover:text-[#eeeeee]"><Check size={14} /></button><button type="button" onClick={() => void forgetMemory(memory)} aria-label={text.forget} className="grid h-7 w-7 shrink-0 place-items-center rounded-md border-0 bg-transparent text-[#777777] hover:bg-[#2b2b2b] hover:text-[#eeeeee]"><Trash2 size={14} /></button></article>)}</section> : <section className="mt-9 py-12 text-center"><strong className="block text-xs font-medium text-[#bdbdbd]">{projectPath ? text.emptyTitle : text.noProject}</strong><p className="mx-auto mt-1 max-w-[360px] text-[11px] leading-relaxed text-[#777777]">{projectPath && (query ? text.emptyTitle : text.emptyDescription)}</p></section>}
  </main>;
}
