...
<AnimatePresence initial={false}>{showFileTree && <motion.aside initial={{ width: 0, opacity: 0 }} animate={{ width: 374, opacity: 1 }} exit={{ width: 0, opacity: 0 }} transition={{ type: 'spring', stiffness: 420, damping: 34 }} className="flex w-[374px] shrink-0 flex-col overflow-hidden border-l border-[#2b2b2b] bg-[#171717]"><div className="min-h-0 flex-1 overflow-auto px-3 py-3 [scrollbar-width:none]">{tree.length ? renderTree(tree) : <div className="p-3 text-sm text-[#777777]">No se encontraron archivos.</div>}</div></motion.aside>}</AnimatePresence>
    </div>}
  </div>;
}
